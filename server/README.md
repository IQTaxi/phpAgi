...
### Copy all files from /py_scipts/ to /usr/local/bin/
```
...
[from-internal]
exten => 4017,1,Set(UNIQ=${UNIQUEID})
 same => n,Set(FILEBASE=/tmp/${CALLERID(num)})
 same => n,Wait(1)
 same => n,Playback(custom/welcome-v2)
 same => n,Set(NAME_SIZE=0)
 same => n(name_retry),Playback(custom/give-mame-v2)
 same => n,Record(${FILEBASE}/name-${UNIQ}.wav16,2,10)
 same => n,System(/usr/local/bin/send_to_google_stt.py ${FILEBASE}/name-${UNIQ}.wav16 > ${FILEBASE}/name-${UNIQ}.txt)
 same => n,Wait(1)
 same => n,Set(NAME_SIZE=${STAT(s,${FILEBASE}/name-${UNIQ}.txt)})
 same => n,GotoIf($[${NAME_SIZE} < 2]?name_retry)

 same => n,Set(PICKUP_TXT_SIZE=0)
 same => n(pickup_retry),Playback(custom/give-pickup-address-v2)
 same => n,Record(${FILEBASE}/pickup-${UNIQ}.wav16,2,10)
 same => n,System(/usr/local/bin/send_to_google_stt.py ${FILEBASE}/pickup-${UNIQ}.wav16 > ${FILEBASE}/pickup-${UNIQ}.txt)
 same => n,System(/usr/local/bin/fetch_latlng_google.py ${FILEBASE}/pickup-${UNIQ}.txt > ${FILEBASE}/location-pickup-${UNIQ}.txt)
 same => n,Wait(1)
 same => n,Set(PICKUP_TXT_SIZE=${STAT(s,${FILEBASE}/pickup-${UNIQ}.txt)})
 same => n,GotoIf($[${PICKUP_TXT_SIZE} < 2]?pickup_retry)
 same => n,Set(PICKUP_LOC_SIZE=${STAT(s,${FILEBASE}/location-pickup-${UNIQ}.txt)})
 same => n,GotoIf($[${PICKUP_LOC_SIZE} < 2]?pickup_retry)

 same => n,Set(DEST_TXT_SIZE=0)
 same => n(dest_retry),Playback(custom/give-dest-address-v2)
 same => n,Record(${FILEBASE}/dest-${UNIQ}.wav16,2,10)
 same => n,System(/usr/local/bin/send_to_google_stt.py ${FILEBASE}/dest-${UNIQ}.wav16 > ${FILEBASE}/dest-${UNIQ}.txt)
 same => n,System(/usr/local/bin/fetch_latlng_google.py ${FILEBASE}/dest-${UNIQ}.txt > ${FILEBASE}/location-dest-${UNIQ}.txt)
 same => n,Wait(1)
 same => n,Set(DEST_TXT_SIZE=${STAT(s,${FILEBASE}/dest-${UNIQ}.txt)})
 same => n,GotoIf($[${DEST_TXT_SIZE} < 2]?dest_retry)
 same => n,Set(DEST_LOC_SIZE=${STAT(s,${FILEBASE}/location-dest-${UNIQ}.txt)})
 same => n,GotoIf($[${DEST_LOC_SIZE} < 2]?dest_retry)

; --- CONFIRMATION ---
 same => n(confirm),System(wget -q -O ${FILEBASE}/confirm-${UNIQ}.mp3 "http://188.245.212.246:221/tts?text=Παρακαλώ επιβεβαιώστε. Όνομα: $(cat ${FILEBASE}/name-${UNIQ}.txt). Παραλαβή: $(cat ${FILEBASE}/pickup-${UNIQ}.txt). Προορισμός: $(cat ${FILEBASE}/dest-${UNIQ}.txt)&lang=el")
 same => n,MP3Player(${FILEBASE}/confirm-${UNIQ}.mp3)

 same => n,System(wget -q -O ${FILEBASE}/options-${UNIQ}.mp3 "http://188.245.212.246:221/tts?text=Πατήστε 0 για επιβεβαίωση, 1 για αλλαγή ονόματος, 2 για αλλαγή παραλαβής, 3 για αλλαγή προορισμού&lang=el")
 same => n,Read(DTMF_OPTION,${FILEBASE}/options-${UNIQ},1,,,5)

 same => n,GotoIf($["${DTMF_OPTION}" = "0"]?do_register)
 same => n,GotoIf($["${DTMF_OPTION}" = "1"]?name_retry_confirmed)
 same => n,GotoIf($["${DTMF_OPTION}" = "2"]?pickup_retry_confirmed)
 same => n,GotoIf($["${DTMF_OPTION}" = "3"]?dest_retry_confirmed)
 same => n,Playback(custom/invalid-v2)
 same => n,Goto(confirm)

; --- RETRY PATHS AFTER CONFIRM ---
 same => n(name_retry_confirmed),Playback(custom/give-mame-v2)
 same => n,Record(${FILEBASE}/name-${UNIQ}.wav16,2,10)
 same => n,System(/usr/local/bin/send_to_google_stt.py ${FILEBASE}/name-${UNIQ}.wav16 > ${FILEBASE}/name-${UNIQ}.txt)
 same => n,Wait(1)
 same => n,Set(NAME_SIZE=${STAT(s,${FILEBASE}/name-${UNIQ}.txt)})
 same => n,GotoIf($[${NAME_SIZE} < 2]?name_retry_confirmed)
 same => n,Goto(confirm)

 same => n(pickup_retry_confirmed),Playback(custom/give-pickup-address-v2)
 same => n,Record(${FILEBASE}/pickup-${UNIQ}.wav16,2,10)
 same => n,System(/usr/local/bin/send_to_google_stt.py ${FILEBASE}/pickup-${UNIQ}.wav16 > ${FILEBASE}/pickup-${UNIQ}.txt)
 same => n,System(/usr/local/bin/fetch_latlng_google.py ${FILEBASE}/pickup-${UNIQ}.txt > ${FILEBASE}/location-pickup-${UNIQ}.txt)
 same => n,Wait(1)
 same => n,Set(PICKUP_TXT_SIZE=${STAT(s,${FILEBASE}/pickup-${UNIQ}.txt)})
 same => n,GotoIf($[${PICKUP_TXT_SIZE} < 2]?pickup_retry_confirmed)
 same => n,Set(PICKUP_LOC_SIZE=${STAT(s,${FILEBASE}/location-pickup-${UNIQ}.txt)})
 same => n,GotoIf($[${PICKUP_LOC_SIZE} < 2]?pickup_retry_confirmed)
 same => n,Goto(confirm)

 same => n(dest_retry_confirmed),Playback(custom/give-dest-address-v2)
 same => n,Record(${FILEBASE}/dest-${UNIQ}.wav16,2,10)
 same => n,System(/usr/local/bin/send_to_google_stt.py ${FILEBASE}/dest-${UNIQ}.wav16 > ${FILEBASE}/dest-${UNIQ}.txt)
 same => n,System(/usr/local/bin/fetch_latlng_google.py ${FILEBASE}/dest-${UNIQ}.txt > ${FILEBASE}/location-dest-${UNIQ}.txt)
 same => n,Wait(1)
 same => n,Set(DEST_TXT_SIZE=${STAT(s,${FILEBASE}/dest-${UNIQ}.txt)})
 same => n,GotoIf($[${DEST_TXT_SIZE} < 2]?dest_retry_confirmed)
 same => n,Set(DEST_LOC_SIZE=${STAT(s,${FILEBASE}/location-dest-${UNIQ}.txt)})
 same => n,GotoIf($[${DEST_LOC_SIZE} < 2]?dest_retry_confirmed)
 same => n,Goto(confirm)

; --- FINAL REGISTER STEP ---
 same => n(do_register),System(/usr/local/bin/register_call.py ${CALLERID(num)} ${FILEBASE}/name-${UNIQ}.txt ${FILEBASE}/pickup-${UNIQ}.txt ${FILEBASE}/location-pickup-${UNIQ}.txt ${FILEBASE}/dest-${UNIQ}.txt ${FILEBASE}/location-dest-${UNIQ}.txt > ${FILEBASE}/register-${UNIQ}.txt)
 same => n,System(wget -q -O ${FILEBASE}/register-${UNIQ}.mp3 "http://188.245.212.246:221/tts?text=$(cat ${FILEBASE}/register-${UNIQ}.txt)&lang=el")
 same => n,MP3Player(${FILEBASE}/register-${UNIQ}.mp3)
 same => n,Hangup()
EOF
