...
/**
 * Constants for the AGI Call Handler System
 * Centralizes all magic numbers, DTMF codes, and status codes
 */
class CallHandlerConstants
{
    // DTMF Input Codes
    const DTMF_IMMEDIATE_CALL = '1';
    const DTMF_RESERVATION = '2';
    const DTMF_OPERATOR = '3';
    const DTMF_LANGUAGE_ENGLISH = '9';
    const DTMF_CONFIRM_YES = '0';
    const DTMF_CONFIRM_NO = '1';
    const DTMF_USE_SAVED_ADDRESS = '1';
    const DTMF_NEW_ADDRESS = '2';
    ...
    // Timeouts
    const TIMEOUT_DTMF_SECONDS = 10;
    const TIMEOUT_RECORDING_MS = 10000;
    const TIMEOUT_API_SECONDS = 30;

    // IQTaxi API Status Codes
    const STATUS_SEARCHING = -1;
    const STATUS_COMING = 1;
    const STATUS_ARRIVED = 2;
    const STATUS_PICKING_UP = 3;
    const STATUS_DROPPING_OFF = 8;
    const STATUS_DRIVER_ACCEPTED = 10;
    const STATUS_NO_TAXI_FOUND = 20;
    const STATUS_CANCELLED_BY_PASSENGER = 30;
    const STATUS_CANCELLED_BY_DRIVER = 31;
    const STATUS_CANCELLED_BY_SYSTEM = 32;
    const STATUS_PAYMENT_REGISTERED = 40;
    const STATUS_WILL_ARRIVE = 50;
    const STATUS_WAITING_FOR_DRIVER = 60;
    const STATUS_DRIVER_RESPONDED = 70;
    const STATUS_RESERVATION_MODIFIED = 80;
    const STATUS_TRIP_COMPLETED = 100;
    const STATUS_NEW_MESSAGE = 101;
    const STATUS_ON_THE_WAY = 255;

    // Final statuses that end status monitoring
    const FINAL_STATUSES = [30, 31, 32, 100, 255];

    // Validation
    const MIN_NAME_LENGTH = 2;
    const MIN_ADDRESS_LENGTH = 3;
    const MIN_AUDIO_FILE_SIZE = 100;
    const MIN_RECORDING_SIZE = 1000;

    // Auto Call Centers Modes
    const MODE_ALL_DISABLED = 0;
    const MODE_ASAP_ONLY = 1;
    const MODE_RESERVATIONS_ONLY = 2;
    const MODE_ALL_ENABLED = 3;
}

/**
 * Data Collection Helper
 * Eliminates code duplication in name/pickup/destination collection
 */
class DataCollectionHelper
{
     ...
     */
    public static function isValidInput($input, $minLength)
    {
        return !empty($input) && strlen(trim($input)) >= $minLength;
    }

    /**
     * Validate and prepare recording file path
     * @param string $filebase Base directory for recordings
     * @param string $type Type of recording (name, pickup, destination)
     * @param int $attempt Current attempt number
     * @return string Full path to recording file
     */
    public static function getRecordingPath($filebase, $type, $attempt)
    {
        return "{$filebase}/recordings/{$type}_{$attempt}";
    }

    /**
     * Check if recording file was created and is valid
     * @param string $wavFile Path to WAV file
     * @param int $minSize Minimum file size in bytes
     * @return bool True if file exists and meets size requirement
     */
    public static function isValidRecording($wavFile, $minSize = CallHandlerConstants::MIN_RECORDING_SIZE)
    {
        return file_exists($wavFile) && filesize($wavFile) >= $minSize;
    }
}

/**
 * Automated Taxi Call Registration System (Refactored v2)
 *
 ...
 *
 * Features:
 * - Greek/English/Bulgarian speech-to-text recognition via Google Cloud STT
 * - Text-to-speech confirmation via Google Cloud TTS or Edge TTS
 * - Address geocoding via Google Maps Geocoding API or Places API v1
 * - Call registration via IQTaxi API
 * - User data management with pickup address confirmation
 * - Real-time status monitoring with callback support
 *
 * @author AGI Call Handler Team
 * @version Production 2.0 (Refactored)
 */
class AGICallHandler2
{
    // === PROPERTIES ===
    private $agi_env = [];
    private $uniqueid = '';
...
/**
 * Initialize and run the improved call handler (v2)
 */
try {
    error_log("AGI v2: Starting main execution");
    $call_handler = new AGICallHandler2();
    error_log("AGI v2: Call handler created, starting runCallFlow");
    $call_handler->runCallFlow();
    error_log("AGI v2: runCallFlow completed successfully, exiting with code 0");
    exit(0); // Exit successfully after call flow completes
} catch (Exception $e) {
    error_log("Fatal error in AGI Call Handler v2: " . $e->getMessage());
    error_log("AGI v2: Stack trace: " . $e->getTraceAsString());
    echo "HANGUP\n";
    exit(1); // Exit with error code on exception
} catch (Error $e) {
    error_log("Fatal PHP error in AGI Call Handler v2: " . $e->getMessage());
    error_log("AGI v2: Stack trace: " . $e->getTraceAsString());
    echo "HANGUP\n";
