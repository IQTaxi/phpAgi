...
class CallMerger
{
    ...
    /**
     * Main function to merge two calls sequentially with event monitoring
     */
     ...
     */
    public function mergeCallsSimple($phone1, $phone2, $conference_room = null, $wait_time = 8)
    {
        try {
            // Generate conference room if not provided
            if (!$conference_room) {
            ...
            echo "Starting simple timed call merge: $phone1 -> $phone2 in conference room $conference_room\n";

            // Step 1: Originate first call
            echo "Step 1: Calling $phone1...\n";
            $actionid1 = $this->originateCall($phone1, 'call-merge-wait', $conference_room, 1, 'Conference Call');
            ...
            echo "Step 3 complete: Second call initiated (ActionID: $actionid2)\n";
     ...
     */
    public function mergeCallsWithCallFile($phone1, $phone2, $conference_room = null, $wait_time = 8)
    {
        try {
            // Generate conference room if not provided
            if (!$conference_room) {
            ...
            echo "Starting call merge with call file method: $phone1 -> $phone2 in conference room $conference_room\n";
            ...
            $this->createCallFile($phone2, 'call-merge', $conference_room);

            echo "Step 3 complete: Call file created for second call\n";
    ...
    {
        $callfile_content = "Channel: SIP/{$number}\n";
        ...
        echo "Call file created: $spool_file\n";
        return $spool_file;
    }
...
if (isset($_GET['phone1']) && isset($_GET['phone2'])) {
    ...
    $phone1 = $_GET['phone1'];
    $phone2 = $_GET['phone2'];
    $conference_room = isset($_GET['conference_room']) ? $_GET['conference_room'] : null;
    $method = isset($_GET['method']) ? $_GET['method'] : 'simple';

    $merger = new CallMerger();

    if ($method === 'simple') {
        $wait_time = isset($_GET['wait_time']) ? (int)$_GET['wait_time'] : 8;
        $result = $merger->mergeCallsSimple($phone1, $phone2, $conference_room, $wait_time);
    } elseif ($method === 'callfile') {
        $wait_time = isset($_GET['wait_time']) ? (int)$_GET['wait_time'] : 8;
        $result = $merger->mergeCallsWithCallFile($phone1, $phone2, $conference_room, $wait_time);
    } else {
...
if (!isset($_GET['phone1']) && !isset($_POST['phone1']) && !isset($argv[1])) {
    ...
    <body>
        ...
        <code>call_merge.php?phone1=20&phone2=6974888710&method=simple&wait_time=8</code>
        ...
        <ul>
            <li><strong>GET Method:</strong> Add parameters to URL: ?phone1=XXX&phone2=YYY&method=simple</li>
            <li><strong>POST Method:</strong> Use the form above</li>
        ...
        <ul>
            ...
            <li><code>call_merge.php?phone1=1234567890&phone2=0987654321&method=simple</code></li>
            <li><code>call_merge.php?phone1=1234567890&phone2=0987654321&method=callfile&wait_time=10</code></li>
        </ul>
    </body>
    </html>';
}
