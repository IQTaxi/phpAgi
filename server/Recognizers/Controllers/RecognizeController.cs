...
namespace Recognizers.Controllers
{
    ...
    public class RecognizeController : ControllerBase
    {
        ...
        private static string NormalizeGreekTime(string input)
        {
            // Pattern: number + time of day indicator → number + πμ/μμ
            var patterns = new (string pattern, string replacement)[]
            {
                // "στις X το πρωί" → "στις X πμ"
                (@"στις\s+(\d{1,2})(:\d{2})?\s+το\s+πρωί", "στις $1$2 πμ"),
                (@"στις\s+(\d{1,2})(:\d{2})?\s+το\s+πρωι", "στις $1$2 πμ"),

                // "στις X το μεσημέρι" → "στις X μμ"
                (@"στις\s+(\d{1,2})(:\d{2})?\s+το\s+μεσημέρι", "στις $1$2 μμ"),
                (@"στις\s+(\d{1,2})(:\d{2})?\s+το\s+μεσημερι", "στις $1$2 μμ"),

                // "στις X το απόγευμα" → "στις X μμ"
                (@"στις\s+(\d{1,2})(:\d{2})?\s+το\s+απόγευμα", "στις $1$2 μμ"),
                (@"στις\s+(\d{1,2})(:\d{2})?\s+το\s+απογευμα", "στις $1$2 μμ"),

                // "στις X το βράδυ" → "στις X μμ"
                (@"στις\s+(\d{1,2})(:\d{2})?\s+το\s+βράδυ", "στις $1$2 μμ"),
                (@"στις\s+(\d{1,2})(:\d{2})?\s+το\s+βραδυ", "στις $1$2 μμ"),

                // "στις X τη νύχτα" → "στις X μμ"
                (@"στις\s+(\d{1,2})(:\d{2})?\s+τη\s+νύχτα", "στις $1$2 μμ"),
                (@"στις\s+(\d{1,2})(:\d{2})?\s+τη\s+νυχτα", "στις $1$2 μμ"),
                (@"στις\s+(\d{1,2})(:\d{2})?\s+το\s+βράδυ", "στις $1$2 μμ"),

                // Without "στις" - "X το πρωί" etc.
                (@"(\d{1,2})(:\d{2})?\s+το\s+πρωί", "$1$2 πμ"),
                (@"(\d{1,2})(:\d{2})?\s+το\s+πρωι", "$1$2 πμ"),
                (@"(\d{1,2})(:\d{2})?\s+το\s+μεσημέρι", "$1$2 μμ"),
                (@"(\d{1,2})(:\d{2})?\s+το\s+μεσημερι", "$1$2 μμ"),
                (@"(\d{1,2})(:\d{2})?\s+το\s+απόγευμα", "$1$2 μμ"),
                (@"(\d{1,2})(:\d{2})?\s+το\s+απογευμα", "$1$2 μμ"),
                (@"(\d{1,2})(:\d{2})?\s+το\s+βράδυ", "$1$2 μμ"),
                (@"(\d{1,2})(:\d{2})?\s+το\s+βραδυ", "$1$2 μμ"),
                (@"(\d{1,2})(:\d{2})?\s+τη\s+νύχτα", "$1$2 μμ"),
                (@"(\d{1,2})(:\d{2})?\s+τη\s+νυχτα", "$1$2 μμ"),

                // Handle "πρωί-πρωί", "νωρίς το πρωί" etc.
                (@"νωρίς\s+το\s+πρωί", "6 πμ"),
                (@"νωρις\s+το\s+πρωι", "6 πμ"),
                (@"αργά\s+το\s+βράδυ", "11 μμ"),
                (@"αργα\s+το\s+βραδυ", "11 μμ"),
                (@"αργά\s+τη\s+νύχτα", "11 μμ"),
                (@"αργα\s+τη\s+νυχτα", "11 μμ"),

                // Standalone time of day (when no number specified)
                (@"\bτο\s+μεσημέρι\b", "12 μμ"),
                (@"\bτο\s+μεσημερι\b", "12 μμ"),
                (@"\bτα\s+μεσάνυχτα\b", "00:00"),
                (@"\bτα\s+μεσανυχτα\b", "00:00"),
                (@"\bμεσάνυχτα\b", "00:00"),
                (@"\bμεσανυχτα\b", "00:00"),

                // "X και μισή το πρωί/απόγευμα" etc.
                (@"(\d{1,2})\s+και\s+μισή?\s+το\s+πρωί", "$1:30 πμ"),
                (@"(\d{1,2})\s+και\s+μισή?\s+το\s+πρωι", "$1:30 πμ"),
                (@"(\d{1,2})\s+και\s+μισή?\s+το\s+μεσημέρι", "$1:30 μμ"),
                (@"(\d{1,2})\s+και\s+μισή?\s+το\s+μεσημερι", "$1:30 μμ"),
                (@"(\d{1,2})\s+και\s+μισή?\s+το\s+απόγευμα", "$1:30 μμ"),
                (@"(\d{1,2})\s+και\s+μισή?\s+το\s+απογευμα", "$1:30 μμ"),
                (@"(\d{1,2})\s+και\s+μισή?\s+το\s+βράδυ", "$1:30 μμ"),
                (@"(\d{1,2})\s+και\s+μισή?\s+το\s+βραδυ", "$1:30 μμ"),

                // "X και τέταρτο"
                (@"(\d{1,2})\s+και\s+τέταρτο\s+το\s+πρωί", "$1:15 πμ"),
                (@"(\d{1,2})\s+και\s+τέταρτο\s+το\s+απόγευμα", "$1:15 μμ"),
                (@"(\d{1,2})\s+και\s+τέταρτο\s+το\s+βράδυ", "$1:15 μμ"),
                (@"(\d{1,2})\s+και\s+τεταρτο", "$1:15"),

                // "X παρά τέταρτο"
                (@"(\d{1,2})\s+παρά\s+τέταρτο\s+το\s+πρωί", "$1:45 πμ"),
                (@"(\d{1,2})\s+παρά\s+τέταρτο\s+το\s+απόγευμα", "$1:45 μμ"),
                (@"(\d{1,2})\s+παρά\s+τέταρτο\s+το\s+βράδυ", "$1:45 μμ"),
                (@"(\d{1,2})\s+παρα\s+τεταρτο", "$1:45"),
            };
